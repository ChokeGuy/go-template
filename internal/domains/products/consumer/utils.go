...
func (s *ProductMessageProcessor) CommitMessage(ctx context.Context, r *kafka.Consumer, m *kafka.Message) {
...
func (s *ProductMessageProcessor) commitErrMessage(ctx context.Context, r *kafka.Consumer, m *kafka.Message) {
	s.log.KafkaLogCommittedMessage(*m.TopicPartition.Topic, int(m.TopicPartition.Partition), int64(m.TopicPartition.Offset))
	if _, err := r.CommitMessage(m); err != nil {
		s.log.WarnMsg("commitMessage", err)
	}
